  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="grid grid-cols-1 gap-3 sm:gap-4 sm:grid-cols-2 lg:grid-cols-5">
        ...
        <div className="rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-4 sm:p-5 shadow-sm hover:shadow-md transition-shadow">
          <div className="flex items-center justify-between">
            <div className="text-xs sm:text-sm text-gray-500 dark:text-gray-400">Budgets</div>
          ...
          <div className="text-xs text-gray-500 dark:text-gray-400 mt-1">
            {budgets.filter(b => b.budget === 0).length > 0 ? (
              <span className="text-amber-600 dark:text-amber-400">{budgets.filter(b => b.budget === 0).length} pending setup</span>
            ) : (
                ...
                <PieChart>
                  ...
                  >
                    {categoryData.map((entry, index) => {
                      // Try to find matching budget to get category color
                      const matchingBudget = budgets.find(b => b.category?.name === entry.name)
        ...
        <div className="rounded-lg border border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-900/20 p-3 sm:p-4 shadow-sm">
          <div className="flex items-center justify-between mb-3 sm:mb-4">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-sm sm:text-base font-medium text-amber-900 dark:text-amber-200">Budgets Pending Setup</h2>
            </div>
            <Link 
              to="/budgets" 
          ...
          <p className="text-xs sm:text-sm text-amber-800 dark:text-amber-300 mb-3">
            These budgets were auto-created but need budget amounts to be set:
          </p>
      ...
      {budgets.filter(b => b.budget > 0).length > 0 && (
        <div className="rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-3 sm:p-4 shadow-sm">
          <div className="flex items-center justify-between mb-3 sm:mb-4">
            <h2 className="text-sm sm:text-base font-medium text-gray-900 dark:text-white">Active Budgets</h2>
            <Link 
              to="/budgets" 
              className="text-xs sm:text-sm text-blue-600 dark:text-blue-400 hover:underline"
            >
